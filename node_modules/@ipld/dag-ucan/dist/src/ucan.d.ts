 ...
 */
export interface Authority<A extends number = number> extends Identity, Verifier<A> {
}
/** The {@link Identity} that can issue / delegate UCAN by signing */
export interface Issuer<A extends number = number> extends Signer<A>, Identity {
}
/** The version of the UCAN spec used to produce a specific UCAN. */
export declare type Version = `${number}.${number}.${number}`;
export declare type SignPayload<C extends Capability> = ToString<ByteView<[Headers, Payload<C>]>, `${ToString<Header>}.${ToString<Payload<C>>}`>;
/**
...
], `${ToString<Header>}.${ToString<Payload<C>>}.${ToString<Signature<SignPayload<C>>>}`>;
/**
 ...
 */
export declare type Proof<C extends Capability = Capability, A extends number = number> = Link<Model<C>, typeof CBOR_CODE, A> | Link<JWT<C>, typeof RAW_CODE, A>;
/**
 ...
 */
export interface Block<T extends unknown = unknown, C extends number = number, A extends number = number, V extends CIDVersion = 1> {
    bytes: ByteView<T>;
    cid: Link<T, C, A, V>;
}
/**
 ...
 */
export declare type Resource = `${string}:${string}`;
/**
 ...
 *
 */
export interface Capability<Can extends Ability = Ability, With extends Resource = Resource> {
    with: With;
    can: Can;
}
/**
 ...
 * "can" and "with".
 */
export declare type Constraints<C extends Capability> = Omit<C, "can" | "with">;
/**
 ...
 */
export interface Link<T extends unknown = unknown, C extends number = number, A extends number = number, V extends CIDVersion = 1> extends CID<C, A, V>, Phantom<T> {
}
/**
 ...
 * @template V - CID version
 */
export interface CID<C extends number = number, A extends number = number, V extends CIDVersion = CIDVersion> {
    readonly version: V;
    readonly code: C;
